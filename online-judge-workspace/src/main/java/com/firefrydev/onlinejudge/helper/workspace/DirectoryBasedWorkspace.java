...
public class DirectoryBasedWorkspace implements Workspace {
    ...
    public DirectoryBasedWorkspace(WorkspaceProvider provider) {
        this.provider = provider;
        this.onlineJudgeSystem = provider.getOnlineJudgeSystem();
        this.workspaceDir = provider.getWorkspaceDir();
    ...
    public TestResult[] test() {
        ...
        try {
            ...
            try {
                ...
                return new TestResult[] {TestResult.failed("unknown", "Failed to load solution class", e2)};
            }
        }
    ...
    public TestResult[] test(Class solution) {
        try {
            ...
            return new TestResult[] {TestResult.failed("unknown", "Exception while testing the solution class", e)};
        }
    }
    ...
    private TestResult getTestResult(File test, String output, String expectedOutput) {
        ...
        for (i = 0; i < Math.min(lines1.length, lines2.length); i++) {
            if (!lines1[i].equals(lines2[i])) {
                return TestResult.failed(test.getName(), "Outputs are not equals" +
                        "\nExpected: " + lines2[i] +
                        "\nFound:    " + lines1[i]);
            }
        }
